...
- **Coordinate Projection & Buffer**: Viewport selection box converts to native image coordinates using:
  `cropX = clamp(0, naturalWidth - 1, round((screenBox.left - imgRect.left) * scaleX - buffer))` with an 8px safety buffer.
...
| `cleanOcrText()` | `src/content/ocr/text-cleaner.js` | Strips pipes (`\|`), cleans chevrons, normalizes months & currency |
...
| `calculateImageCropBounds()` / `startLightboxCrop()` | `src/content/ui/lightbox-crop.js` | Lightbox coordinate projection with 8px buffer and rAF drag selection |
